...
The clothing chat and alteration search are separate sections. The chat message history scrolls above its composer, and a visually distinct alteration section follows the complete chat panel after the message text box. The alteration section is part of the normal page flow and has no internal scroll area. It offers two explicit ways to find alteration support: use the browser's current location or submit a town, city, or postcode. Search results and location errors appear only inside the alteration section and are never added to the chat. Current-location access always requires the browser's permission prompt. Precise coordinates are sent through the Worker to OpenStreetMap only for the submitted search; they are not stored in chat history or sent to the LLM. The current results are kept only in page memory until the user searches again, clears the page, or refreshes it.
